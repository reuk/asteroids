...
public:
  ...
  template <typename... Ts> static void log(Ts &&... ts) {
    auto str = get_string(ts...);
    std::ofstream of(fname, std::ofstream::app);
    of << str << std::flush;
  }

  template <typename... Ts> static void log_err(Ts &&... ts) {
    auto str = get_string(ts...);
    std::ofstream of(fname, std::ofstream::app);
    of << str << std::flush;
...
private:
  ...
  template <typename... Ts> static std::string get_string(Ts &&... ts) {
    std::stringstream ss;
    // auto t0 =
  ...
  template <typename T> static void build_string(std::stringstream &ss, T &&t) {
    ss << t;
  }
