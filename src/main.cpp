...
public:
  Asteroids()
      :
        ...
        frame_count(0),
#endif
        screen_boundary(shader_program), ship_graphic(shader_program),
        bullet_graphic(shader_program), ship(ship_graphic, bullet_graphic),
        life_counter(ship_graphic, 0.02, vec2(-0.9, -0.9)), lives(max_lives) {

    for (auto i = 0; i != 20; ++i) {
      asteroid_graphic.emplace_back(move(AsteroidGraphic(shader_program)));
    }
  ...
  template <typename T> void clear_marked(vector<T> &t, vector<bool> &marked) {
    auto i = t.begin();
  ...
  template <typename T> void split_marked(vector<T> &t, vector<bool> &marked) {
    vector<T> to_insert;
  ...
  void update() override {
    ...
    //  check for collisions
    {
      ...
      for (; i != asteroids.end(); ++i, ++id) {
        if (*id) {
          particle_system.emplace_back(
              move(ParticleSystem(shader_program, i->position.get_current())));
        }
      }
  ...
  void add_asteroid() {
    ...
    uniform_int_distribution<decltype(asteroid_graphic.size())> graphic_dist(
        0, asteroid_graphic.size() - 1);
