...
void Window::default_hints() { glfwDefaultWindowHints(); }

void Window::hint(int a, int b) { glfwWindowHint(a, b); }
...
void Window::set_title(const char *a) { glfwSetWindowTitle(window.get(), a); }

void Window::get_pos(int &a, int &b) const {
  glfwGetWindowPos(window.get(), &a, &b);
}

void Window::set_pos(int a, int b) { glfwSetWindowPos(window.get(), a, b); }

void Window::get_size(int &a, int &b) const {
  glfwGetWindowSize(window.get(), &a, &b);
}

void Window::set_size(int a, int b) { glfwSetWindowSize(window.get(), a, b); }
...
void Window::iconify() { glfwIconifyWindow(window.get()); }

void Window::restore() { glfwRestoreWindow(window.get()); }

void Window::show() { glfwShowWindow(window.get()); }

void Window::hide() { glfwHideWindow(window.get()); }
...
Window::set_framebuffer_size_callback(GLFWframebuffersizefun a) {
...
void Window::swap_buffers() { glfwSwapBuffers(window.get()); }

void Window::make_context_current() { glfwMakeContextCurrent(window.get()); }
