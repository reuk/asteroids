...
Particle::Particle()
    : Mover(vec2(0.0f), random_angle() * speed_dist(engine)), lifetime(1.0f) {}
...
bool Particle::is_dead() const { return lifetime < 0.0f; }
...
ParticleSystem::ParticleSystem(GenericShader &shader_program,
                               const vec2 &position)
    : shader_program(shader_program), position(position),
      particles(starting_particles), size(starting_particles) {
...
bool ParticleSystem::is_dead() const { return particles.empty(); }
